...
def test_skyfield_time_to_astropy():
    ts = load.timescale()                            # type skyfield.timelib.Timescale
    t_sf = ts.utc(2019, 9, 30, 23, 5, 6)             # type skyfield.timelib.Time
    t_apy = almanac.skyfield_time_to_astropy(t_sf)  # type astropy.time.core.Time
...
def test_astropy_time_to_skyfield():
    ts = load.timescale()
    t_apy = Time('2019-09-30 23:05:06', scale='utc')
    t_sf = almanac.astropy_time_to_skyfield(ts, t_apy)
    assert isinstance(t_sf, skyfield.timelib.Time)
    assert t_sf.utc == (2019, 9, 30, 23, 5, 6.0)
    print()
    print(str(t_sf))
    print(str(t_sf.utc))
...
def test_moon_illumination_pct():
    ...
    mi = almanac.moon_illumination_pct(an.master_eph, an.timescale,
                                       Time('2022-02-09 07:16:00'))
...
def test_target_transit_time():
    ...
    transit = almanac.target_transit_time(an.obs, an.master_eph, an.timescale, sc,
                                          target_time)
...
def test_find_target_up_down():
    ...
    with pytest.raises(ValueError):
        _ = almanac.find_target_up_down(an.obs, an.master_eph, an.master_eph['moon'],
                                        an.timescale,
    ...
    moon_up = almanac.find_target_up_down(an.obs, an.master_eph, an.master_eph['moon'],
                                          an.timescale, test_timespan, 'up',
    ...
    moon_down = almanac.find_target_up_down(an.obs, an.master_eph,
                                            an.master_eph['moon'], an.timescale,
                                            test_timespan, 'down',
                                            horizon=almanac.HORIZON_USNO)
    # print(str(moon_down))
    ...
    sun_up = almanac.find_target_up_down(an.obs, an.master_eph, an.master_eph['sun'],
                                         an.timescale, test_timespan, 'up',
                                         horizon=almanac.HORIZON_USNO)
    # print(str(sun_up))
    ...
    sun_down = almanac.find_target_up_down(an.obs, an.master_eph, an.master_eph['sun'],
                                           an.timescale, test_timespan, 'down',
                                           horizon=almanac.HORIZON_USNO)
    # print(str(sun_down))
    ...
    procyon_up = almanac.find_target_up_down(an.obs, an.master_eph, procyon,
                                             an.timescale, test_timespan, 'up',
                                             horizon=almanac.HORIZON_USNO)
    ...
    procyon_down = almanac.find_target_up_down(an.obs, an.master_eph, procyon,
                                               an.timescale, test_timespan, 'down',
                                               horizon=almanac.HORIZON_USNO)
    ...
    kochab_down = almanac.find_target_up_down(an.obs, an.master_eph, kochab,
                                              an.timescale, test_timespan, 'down',
                                              horizon=almanac.HORIZON_USNO)
    ...
    gatria_down = almanac.find_target_up_down(an.obs, an.master_eph, gatria,
                                              an.timescale, test_timespan, 'down',
                                              horizon=almanac.HORIZON_USNO)
...
def test_calc_timespan_no_sun():
    # Normal case:
    an = make_new_astronight()
    fn = almanac.calc_timespan_no_sun(an.obs, an.master_eph, an.timescale,
                                      an.utc_local_middark)
...
def test_ha_dec_from_az_alt():
    ...
    assert len(result) == 2
    assert result == pytest.approx((57.2093, 53.5034), abs=0.0002)
    assert fn(latitude=32.9, az_alt=(315, 45)) == pytest.approx((-57.2093, 53.5034),
                                                                abs=0.0002)
    # List of tuples (az, alt):
    result = fn(latitude=32.9, az_alt=[(45, 45), (0, 60)])
    assert isinstance(result, list)
    assert len(result) == 2
    assert all([isinstance(x, tuple) for x in result])
    assert result[0] == pytest.approx((57.2093, 53.5034), abs=0.0002)
    assert result[1] == pytest.approx((0, 62.9), abs=0.0002)


__________TEST_CLASS_ASTRONIGHT___________________________________________________ = 0
...
def test_astronight_constructor():
    ...
    assert (an.timespan_no_sun.midpoint - Time('2022-02-09 07:16:02.284')).sec == \
           pytest.approx(0, abs=2)
    assert an.timespan_dark.seconds == pytest.approx(42558.1, abs=2)
    assert (an.timespan_dark.midpoint - Time('2022-02-09 07:16:04.796')).sec == \
           pytest.approx(0, abs=2)
    assert (an.utc_local_middark - Time('2022-02-09 07:16:04.796')).sec == \
           pytest.approx(0, abs=2)
    ...
    assert (an.moon_transit - Time('2022-02-09 01:18:25')).sec == \
           pytest.approx(0, abs=2)
