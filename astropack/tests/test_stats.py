...
def test_class_mixed_model_fit():
    ...
    fit = stats.MixedModelFit(df_model, dep_var='Dep', fixed_vars=['A', 'B', 'C'],
                              group_var='Ran')
    ...
    assert isinstance(fit.df_fixed_effects, pd.DataFrame)
    ...
    assert list(fit.df_fixed_effects['Value']) == \
           pytest.approx([16.648186, 0.946692, 1.959923, 4.069383], abs=0.00001)
    assert list(fit.df_fixed_effects['Stdev']) == \
           pytest.approx([2.844632, 0.142185, 0.134386, 0.145358], abs=0.00001)
    assert list(fit.df_fixed_effects['Tvalue']) == \
           pytest.approx([5.8525, 6.65818, 14.58429, 27.99568], abs=0.0001)
    assert list(fit.df_fixed_effects['Pvalue'] * 10**9) == \
           pytest.approx([4.8426, 0.027724, 0, 0], abs=0.0001)

    # Test random-effect (group) results dataframe:
    assert isinstance(fit.df_random_effects, pd.DataFrame)
    ...
    assert list(fit.df_random_effects['Group']) == \
           pytest.approx([-5.164649, 0.543793, 4.620857], abs=0.00001)

    # Test observation results dataframe:
    assert isinstance(fit.df_observations, pd.DataFrame)
    assert list(fit.df_observations['FittedValue'])[0:4] == \
           pytest.approx([24.809899, 10.130408, 19.834543, 7.758331], abs=0.00001)
    assert list(fit.df_observations['Residual'])[0:4] == \
           pytest.approx([0.490179, -0.786949, 0.58315, -1.23926], abs=0.00001)
    ...
    assert list(predictions_with_1) == list(predictions_with_2)  # verify default.
    assert list(predictions_with_1) == \
           pytest.approx(list(fit.df_observations['FittedValue'])[0:4])
...
def test_class_linear_fit():
    ...
    assert isinstance(fit.df_indep_vars, pd.DataFrame)
    ...
    assert all([std_dev == pytest.approx(exp_std_dev, abs=0.00002)
                for (std_dev, exp_std_dev) in zip(fit.df_indep_vars['Stdev'],
                                                  expected_std_devs)])

    # Test observation dataframe:
    assert isinstance(fit.df_observations, pd.DataFrame)
    assert len(fit.df_observations) == fit.nobs
    assert fit.df_observations.loc[0, 'FittedValue'] == \
           pytest.approx(16.7787, abs=0.0002)
    assert fit.df_observations.loc[78, 'FittedValue'] == \
           pytest.approx(13.7680, abs=0.0002)
    assert fit.df_observations.loc[1, 'Residual'] == \
           pytest.approx(+0.24075, abs=0.0002)
    assert fit.df_observations.loc[79, 'Residual'] == \
           pytest.approx(-1.07459, abs=0.0002)


def test_weighted_mean():
    # Normal cases:
    assert stats.weighted_mean([1, 3, 8], [0, 3, 9]) == \
           (81 / 12, pytest.approx(3.535533), pytest.approx(2.795085))
    ...
    # Values with zero weights are ignored:
    assert stats.weighted_mean([1, 3, 8], [0, 3, 9]) == \
           stats.weighted_mean([3, 8], [3, 9])
    # Only 1 nonzero weight:
    assert stats.weighted_mean([1, 2, 5, 11], [0, 0, 3, 0]) == (5, 0, 0)
